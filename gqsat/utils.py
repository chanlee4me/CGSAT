...
def build_argparser():
    parser = argparse.ArgumentParser()
    #  --nums-variable 50 \
    parser.add_argument("--nums-variable", type=int, default=50, help="Number of variables")
    ...
    # 每一步（每个动作）都会引入一个负奖励（negative reward），目的是通过这种设计引导智能体优化其行为策略
    parser.add_argument(
        "--penalty_size",
        type=float,
        default=0.0001,
...
def make_env(problems_paths, args, test_mode=False):
    ...
    #如果处于测试模式，且参数中有test_max_data_limit_per_set，则将对应的参数赋值给max_data_limit_per_set
    if test_mode and hasattr(args, "test_max_data_limit_per_set"):
        max_data_limit_per_set = args.test_max_data_limit_per_set
    #如果不处于测试模式，且参数中有train_max_data_limit_per_set，则将对应的参数赋值给max_data_limit_per_set
    ...
    return gym.make(
        ...
        max_decisions_cap=args.test_time_max_decisions_allowed if test_mode else args.train_time_max_decisions_allowed
    )


def evaluate(agent, args, include_train_set=False):
    agent.net.eval() # 将神经网络设为评估模式
    problem_sets = ( # 构建评估问题集
        [args.eval_problems_paths]
        if not args.eval_separately_on_each
        else [k for k in args.eval_problems_paths.split(":")]
    )
    if include_train_set: # 可能包含训练集（当 include_train_set=True 时）
        problem_sets.extend(
    ...
    for pset in problem_sets: # 遍历问题集
        eval_env = make_env(pset, args, test_mode=True) # 为每个问题集创建评估环境
        #初始化计数器 (pr) 和记录字典
        DEBUG_ROLLOUTS = None
        pr = 0
        walltime = {}
        ...
        #added by cl reset 中不传参，在创建环境时设置最大决策次数
                ...
                while not done:
                    ...
                    action = agent.act([obs], eps=0) # 使用确定性策略（eps=0）
                    obs, _, done, _ = eval_env.step(action) # 与环境交互直到问题解决

                walltime[eval_env.curr_problem] = time.time() - p_st_time # 计算解题时间
                print(
                    f"It took {walltime[eval_env.curr_problem]} seconds to solve a problem."
                )
                sctr = 1 if eval_env.step_ctr == 0 else eval_env.step_ctr
                ns = eval_env.normalized_score(sctr, eval_env.curr_problem) # 获取标准化分数（可能相对于 MiniSat 等 baseline）
